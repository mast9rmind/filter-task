...
const FilterDropdown = () => {
  const { task, user, setState, filterState, dispatchFilter, searchState, dispatchIsBoolean } = useContext(DataContext)
  ...
  // REVIEW : remove use unused variables
  const searchHandler = () => {
    dispatchIsBoolean({
      type: 'SET_ISSHOW',
      isValid: false
  ...
  const dropdown = searchFilterDropdown.map(item => {
    ...
    return (
      <Dropdown.Item key={item} onClick={onClickHandler} className='hover:rounded-lg'>
        {item}
      </Dropdown.Item>
    )
  })
