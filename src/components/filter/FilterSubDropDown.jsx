...
const FilterSubDropDown = () => {
  ...
  const searchFilterDubDropdown = filterState.filteredSubDropdown.filter(filterSubDropdown =>
    ...
    <>
      ...
      {searchFilterDubDropdown.map(item => (
        <Dropdown.Item key={item.id} className='flex gap-2 hover:rounded-lg'>
          <Checkbox
            className='text-violet-600 focus:ring-1 focus:ring-violet-500 hover:cursor-pointer'
            ref={checkbox}
            id={item.id}
            checked={filterState.selectedFilteredSubDropdown.includes(item.itemSubFilter)}
            value={item.itemSubFilter}
            onChange={filterHandler}
          />
          <Label htmlFor={item.id} className='w-full font-normal hover:cursor-pointer'>
            {item.itemSubFilter}
          </Label>
        </Dropdown.Item>
      ))}
